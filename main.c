...
--*/
{
   ...
   if (SDL_Init(PAL_SDL_INIT_FLAGS) < 0 )
   {
	   //printf("11\n");
   ...
   */
   ...
   //printf("begain PAL_CONFIG_PREFIX %s\n",PAL_CONFIG_PREFIX);
   

   PAL_LoadConfig(TRUE);
   //PAL_LoadConfig(FALSE);
   /*
   printf("PAL_CONFIG_PREFIX %s\n",PAL_CONFIG_PREFIX);
   printf("gConfig.pszSavePath %s\n",gConfig.pszSavePath);
   printf("gConfig.pszGamePath %s\n",gConfig.pszGamePath);
   printf("gConfig.pszFontFile %s\n",gConfig.pszFontFile);
   printf("gConfig.pszLogFile %s\n",gConfig.pszLogFile);
   printf("gConfig.pszScaleQuality %s\n",gConfig.pszScaleQuality);
   printf("gConfig.pszShaderPath %s\n",gConfig.pszShaderPath);
   printf("gConfig.pszMsgFile %s\n",gConfig.pszMsgFile);
   printf("gConfig.pszMIDIClient %s\n",gConfig.pszMIDIClient);
   printf("gConfig.pszShader %s\n",gConfig.pszShader);
   
   printf("gConfig.fEnableGLSL %d\n",gConfig.fEnableGLSL);
   printf("gConfig.fEnableHDR %d\n",gConfig.fEnableHDR);
   printf("gConfig.fFullScreen %d\n",gConfig.fFullScreen);
   printf("gConfig.fKeepAspectRatio %d\n",gConfig.fKeepAspectRatio);
   printf("gConfig.iAudioDevice %d\n",gConfig.iAudioDevice);
   printf("gConfig.iAudioChannels %d\n",gConfig.iAudioChannels);
   printf("gConfig.iSampleRate %d\n",gConfig.iSampleRate);
   printf("gConfig.wAudioBufferSize %d\n",gConfig.wAudioBufferSize);
   printf("gConfig.iOPLSampleRate %d\n",gConfig.iOPLSampleRate);
   printf("gConfig.iResampleQuality %d\n",gConfig.iResampleQuality);
   */
   	...
   	if(argc>1)
	{
		...
		//printf("copy home path is %s \n", path);
		strcat(path, "/");
		strcat(path, argv[1]);
		//printf("combine path is %s \n", path);
		mkdir(path, 0755);
		gConfig.pszSavePath = path;

	}
	else
	{
		mkdir(gConfig.pszSavePath, 0755);
		//printf("creat default path is %s \n", gConfig.pszSavePath);
	}
   
   //gConfig.fFullScreen = 0;
   //gConfig.fEnableGLSL = 0;
   //gConfig.fEnableHDR = 0;
   //gConfig.fKeepAspectRatio = 1;
