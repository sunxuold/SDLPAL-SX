...
#define SAL_RGB565(r,g,b) (uint16_t)((r) << 11 | (g) << 6 | (b))

/*
uint16_t*  RGB256Table[]={SAL_RGB565(0,0,0),
						  SAL_RGB565(16,0,0),
						  SAL_RGB565(0,32,0),
						  SAL_RGB565(16,32,0),
						  SAL_RGB565(0,0,16),
...
# endif
   ...
   //
   //printf("VIDEO_Startup %d --%d\n",gConfig.dwScreenWidth, gConfig.dwScreenHeight);
   	
	//FILE *logfile = NULL;//sunxu test
	//logfile = fopen("./test.txt", "a+");
	//fprintf(logfile,"VIDEO_Startup %d --%d\n",gConfig.dwScreenWidth, gConfig.dwScreenHeight); 
	//fclose(logfile);
   ...
   if (gpScreenReal == NULL)
   {
	...
	//logfile = fopen("./test.txt", "a+");
	//fprintf(logfile,"VIDEO_Startup done\n"); 
	//fclose(logfile);	
...
# if defined(PAL_FORCE_UPDATE_ON_PALETTE_SET)
   {
      ...
      if (SDL_GetTicks() - time > 50)
      {
		  ...
		  printf("PAL_FORCE_UPDATE_ON_PALETTE_SET\n");	
...
--*/
{
   ...
   //printf("VIDEO_ToggleScaleScreen %d, %d\n", PAL_DEFAULT_WINDOW_WIDTH, PAL_DEFAULT_WINDOW_HEIGHT);
...
--*/
{
   ...
   {
	  printf("VIDEO_ToggleFullscreen- %d --%d bpp %d\n",gConfig.dwScreenWidth, gConfig.dwScreenHeight, bpp);		
...
--*/
{
   ...
   printf("VIDEO_ChangeDepth- %d --%d bpp %d\n",w, h, bpp);
...
--*/
{
	...
	// Create the surface
	//
	//printf("VIDEO_CreateCompatibleSizedSurface\n");
...
--*/
{
	//printf("VIDEO_UpdateSurfacePalette\n");
#if SDL_VERSION_ATLEAST(2, 0, 0)
...
--*/
{
	//printf("VIDEO_DrawSurfaceToScreen\n");	
#if SDL_VERSION_ATLEAST(2, 0, 0)
   //
   ...
   //VIDEO_DrawSurfaceToScreen sunxu
    int bytsize= pSurface->format->BitsPerPixel/8;
	uint8_t *dest = (uint8_t *)(gpScreenReal->pixels);
	uint8_t * src = (uint8_t *)(pCompatSurface->pixels);
	if((pCompatSurface->w) < (gpScreenReal->w))
		dest += (gpScreenReal->w-pCompatSurface->w)/2;
	
	if((pCompatSurface->h) < (gpScreenReal->h))
		dest += (gpScreenReal->w)*((gpScreenReal->h)-(pCompatSurface->h))/2;	
	
	for(int i=0;i< pCompatSurface->h;i++)
	{
		if(i==gpScreenReal->h) break;
		for(int j=0;j<pCompatSurface->w; j++)
		{
			if(j==gpScreenReal->w) break;			
			*dest = *src;
			dest++;
			src+=bytsize;
		}
		if((pCompatSurface->w) < (gpScreenReal->w))
			dest+=((gpScreenReal->w)-(pCompatSurface->w));
		else
			src+=((pCompatSurface->w)-(gpScreenReal->w))*bytsize;
	...
	switch (bytsize)
	{
		...
		case 2: //16bit color
		{
			uint16_t * src = (uint16_t *)(pCompatSurface->pixels);
			for(int i=0;i< pCompatSurface->h;i++)
			{
				if(i==gpScreenReal->h) break;
				for(int j=0;j<pCompatSurface->w; j++)
				{
					if(j==gpScreenReal->w) break;
					//memcpy(((gpScreenReal->pixels)+i*gpScreenReal->w+j),((pCompatSurface->pixels)+bytsize*(i*pCompatSurface->w+j)),1);
					dest[i*gpScreenReal->w+j] = src[i*pCompatSurface->w+j];
					
					
					//*((gpScreenReal->pixels)+i*gpScreenReal->w+j) = *((pCompatSurface->pixels)+i*pCompatSurface->w+j); 
				}
			}
